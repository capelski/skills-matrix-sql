(function (js, navigation, ajax, paginatedList, htmlNodes, render) {
  ...
  function getSkills() {
    ...
    skillsPromise.then(function (paginatedList) {
      ...
      state.addSkillsList.results = js.arrayDifference(
        paginatedList.Items,
        state.employee.Skills,
        'Id',
      );
      render.foundSkills(state);
    });
  ...
  function removeEmployee(state, event) {
    js.actionModal(
      '<div>Are you sure you want to delete ' + state.employee.Name + '?</div>',
      'Delete',
    )
      .then(function () {
        state.loading = true;
  ...
  function save(state, event) {
    state.loading = true;
    render(state);
    ajax.save('/api/employees', state.employee).then(function (employee) {
      if (employee) {
