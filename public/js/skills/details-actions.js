(function (js, navigation, ajax, paginatedList, htmlNodes, render) {
  ...
  function getEmployees() {
    ...
    employeesPromise.then(function (paginatedList) {
      state.addEmployeesList.loadPhase = 'loaded';
      state.addEmployeesList.results = js.arrayDifference(
        paginatedList.Items,
        state.skill.Employees,
        'Id',
      );
      render.foundEmployees(state);
    });
  ...
  function save(state, event) {
    state.loading = true;
    render(state);
    ajax.save('/api/skills', state.skill).then(function (skill) {
      if (skill) {
  ...
  paginatedList.attachEvents(
    htmlNodes.addEmployeesList,
  ...
  navigation.register('skill-details-section', function (navigationData) {
    ...
    (state.readOnly = navigationData.readOnly), (state.loading = true);
    render(state);
    ...
    return skillPromise.then(function (skill) {
      state.loading = false;
      if (skill) {
        state.skill = skill;
        state.employeesList.results = state.skill.Employees;
