(function (js, navigation, ajax, paginatedList) {
  ...
  function render() {
    ...
    paginatedList.render(htmlNodes.employeesList, state.employees, {
      elementDrawer: function (employee) {
        return (
          '<li class="list-group-item"><a class="reset" href="#" ' +
          "onclick=\"Navigation.navigate('employee-details-section', {employeeId: " +
          employee.Id +
          ', readOnly: true})">' +
          employee.Name +
    ...
    paginatedList.render(htmlNodes.skillsList, state.skills, {
      elementDrawer: function (skill) {
        return (
          '<li class="list-group-item"><a class="reset" href="#" ' +
          "onclick=\"Navigation.navigate('skill-details-section', {skillId: " +
          skill.Id +
          ', readOnly: true})">' +
          skill.Name +
          '<span class="badge floating">' +
  ...
  navigation.register('home-section', function (navigationData) {
    ...
    ajax.get('/api/employees/getMostSkilled', {}, []).then(function (employees) {
      state.employees.loadPhase = 'loaded';
      state.employees.results = employees;
      render();
    });

    ajax.get('/api/skills/getRarest', {}, []).then(function (skills) {
